...
// Tech Stack Categories — updated from resume
const skills = {
  ...
  "Databases & Caching": ["PostgreSQL", "MongoDB", "MySQL", "Redis (Distributed Locks / Caching)", "Qdrant (Vector DB)"],
  "DevOps & Cloud": ["AWS (EC2, S3, RDS, IAM, CloudWatch, SNS, SQS, VPC, ECR, ALB)", "Docker & Docker Compose", "Kubernetes", "CI/CD Pipelines", "Git / GitHub", "Maven"],
...
// Updated nav links — Experience added
const navLinks = ["About", "Experience", "Skills", "Projects", "Highlights", "Contact"];

const EMAIL = "ritikhedau9@gmail.com";
const PHONE = "+91 82695 19161";
const LOCATION = "Pune, India";
const RESUME_LINK = "https://drive.google.com/file/d/13466HLBk5Fp_itP2wnnt0nwjD-TY48lT/view?usp=drivesdk";
const GITHUB_URL = "https://github.com/ritik-hedau18";
const LINKEDIN_URL = "https://www.linkedin.com/in/ritik-hedau/";
...
function TypedText({ texts }) {
  ...
  useEffect(() => {
    ...
    if (isDeleting) {
      if (displayed === "") {
        timer = setTimeout(() => { setIsDeleting(false); setIndex((prevIndex) => (prevIndex + 1) % texts.length); }, 150);
      } else {
        timer = setTimeout(() => { setDisplayed(currentFullText.substring(0, displayed.length - 1)); }, 30);
      }
    } else {
      if (displayed === currentFullText) {
        timer = setTimeout(() => setIsDeleting(true), 2500);
      } else {
        timer = setTimeout(() => { setDisplayed(currentFullText.substring(0, displayed.length + 1)); }, 70);
      }
    }
    return () => clearTimeout(timer);
  }, [displayed, isDeleting, index, texts]);
  return (
    <span>{displayed}<span className="blink-cursor"></span></span>
  );
}
...
function MatrixRain({ onClose }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { clearInterval(interval); if (onClose) onClose(); }, 5000);
    return () => { clearInterval(interval); clearTimeout(timer); window.removeEventListener('resize', resizeCanvas); };
  }, [onClose]);
  return (
    <canvas ref={canvasRef} style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999, pointerEvents: 'none', opacity: 0.75 }} />
  );
}
    ...
    switch (mainCommand) {
      ...
      case "skills":
        output = [
          ...
          "  Events:   Apache Kafka, RabbitMQ, Event-driven Architectures",
          "  Data:     PostgreSQL, MongoDB, MySQL, Redis Caching & Distributed Locks",
          "  DevOps:   Docker & Docker Compose, Kubernetes, AWS (EC2, S3, RDS, IAM, CloudWatch, SNS, SQS, VPC, ECR, ALB)",
      ...
      case "experience":
        output = [
          ...
          "  Location: Pune, India",
          "  ─────────────────────────────────────────",
          "  Key Work: Multi-tenant SaaS microservices, Kafka event pipelines,",
          "            React dashboards, Docker/Kubernetes, AWS infrastructure,",
      ...
      case "projects":
        output = [
          "Completed Projects:",
          "  * NEXUS:    AI RAG workspace with Jina AI + Qdrant Vector DB",
          "  * VAULT:    Secure digital banking platform with Redis locks & 2FA",
          "  * TRACE:    Real-time fraud detection with 7 microservices + Kafka",
          "  * SRIJAN:   Natural language to Spring Boot project generator",
          "  * BITECRAFT: Full-stack food delivery (Spring Boot 3 + React 19)",
          "  * ATLAS:    LinkedIn-style network with Elasticsearch + Grafana",
          "  * ECHO:     Real-time chat with WebSocket + AES-256 encryption",
          "Type 'help' or explore project cards below."
        ];
        break;
      case "theme":
...
function ArchitectureVisualizer({ name }) {
  const renderNode = (x, y, title, subtitle) => {
    const w = 95; const h = 50;
    return (
      <g key={title}>
        <rect x={x - w / 2} y={y - h / 2} width={w} height={h} rx={8} className="arch-node" />
  ...
  if (name === "NEXUS") {
    return (
      <div className="arch-container">
        <div style={{ fontSize: "0.85rem", color: "#06B6D4", fontFamily: "'Fira Code', monospace", marginBottom: 12, fontWeight: 500 }}>// RAG Pipeline & Similarity Search Flow</div>
  ...
  if (name === "TRACE") {
    return (
      <div className="arch-container">
        <div style={{ fontSize: "0.85rem", color: "#06B6D4", fontFamily: "'Fira Code', monospace", marginBottom: 12, fontWeight: 500 }}>// Real-time Event-Driven Fraud Analysis</div>
        <svg viewBox="0 0 660 120" style={{ width: "100%", height: "auto" }}>
          <Markers />
          <path d="M 107.5 60 L 152.5 60" className="arch-flow-line" markerEnd="url(#arrow)" />
          <path d="M 107.5 60 L 152.5 60" className="arch-flow-pulse" markerEnd="url(#arrow-cyan)" />
  ...
  if (name === "SRIJAN") {
    return (
      <div className="arch-container">
        <div style={{ fontSize: "0.85rem", color: "#06B6D4", fontFamily: "'Fira Code', monospace", marginBottom: 12, fontWeight: 500 }}>// Natural Language to Spring Boot Project Compilation</div>
        <svg viewBox="0 0 660 120" style={{ width: "100%", height: "auto" }}>
          <Markers />
          <path d="M 107.5 60 L 152.5 60" className="arch-flow-line" markerEnd="url(#arrow)" />
          <path d="M 107.5 60 L 152.5 60" className="arch-flow-pulse" markerEnd="url(#arrow-cyan)" />
  ...
  if (name === "BITECRAFT") {
    return (
      <div className="arch-container">
        <div style={{ fontSize: "0.85rem", color: "#06B6D4", fontFamily: "'Fira Code', monospace", marginBottom: 12, fontWeight: 500 }}>// Full-Stack Platform Flow & State Sync</div>
        <svg viewBox="0 0 660 120" style={{ width: "100%", height: "auto" }}>
          <Markers />
          <path d="M 107.5 60 L 152.5 60" className="arch-flow-line" markerEnd="url(#arrow)" />
          <path d="M 107.5 60 L 152.5 60" className="arch-flow-pulse" markerEnd="url(#arrow-cyan)" />
...
function ProjectGridCard({ p, index, onClick }) {
  ...
  return (
    <FadeIn delay={index * 0.06}>
      <div className="project-grid-card" onClick={onClick} onMouseMove={handleMouseMove}>
        {p.logo && (<div className="project-logo-container"><img src={p.logo} alt={`${p.name} Logo`} /></div>)}
        ...
        <div className="project-grid-card-hint"><span>View Details →</span></div>
      </div>
    </FadeIn>
  );
}
...
function ProjectDetailView({ p, onBack }) {
  ...
  return (
    <div className="project-detail-view">
      <button className="project-detail-back-btn" onClick={onBack}>← All Projects</button>
      <div className="glass-panel" style={{ borderRadius: 20, padding: "clamp(24px, 4vw, 44px)", position: "relative", overflow: "hidden", border: "1px solid rgba(243, 244, 246, 0.06)" }}>
        ...
        <div className="project-detail-header" style={{ display: "flex", gap: "28px", alignItems: "center", flexWrap: "wrap", marginBottom: "32px" }}>
          {p.logo && (<div className="project-detail-logo"><img src={p.logo} alt={`${p.name} Logo`} /></div>)}
          <div style={{ flex: 1, minWidth: "250px" }}>
            <span style={{ fontSize: "0.7rem", fontFamily: "'Fira Code', monospace", letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--accent-cyan)", background: "rgba(6, 182, 212, 0.08)", border: "1px solid rgba(6, 182, 212, 0.15)", borderRadius: 4, padding: "4px 10px", display: "inline-block", marginBottom: 10 }}>{p.tag}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
            <div>
              <h4 className="detail-section-title">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>
                Project Overview
              </h4>
            ...
            {(p.problem || p.solution) && (
              <div>
                <h4 className="detail-section-title">
                  <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"></path></svg>
                ...
                <div className="problem-solution-card">
                  <div>
                    <h5 style={{ fontSize: "0.88rem", fontWeight: 700, color: "#EF4444", marginBottom: 10, letterSpacing: "1px", display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "#EF4444" }}></span>THE CHALLENGE
                    </h5>
                  ...
                  <div style={{ borderLeft: "1px solid rgba(243, 244, 246, 0.08)", paddingLeft: "20px" }} className="sol-divider">
                    <h5 style={{ fontSize: "0.88rem", fontWeight: 700, color: "var(--accent-cyan)", marginBottom: 10, letterSpacing: "1px", display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "var(--accent-cyan)" }}></span>THE SOLUTION
                    </h5>
            ...
            {p.hasGithub && ["NEXUS", "TRACE", "SRIJAN", "BITECRAFT"].includes(p.name) && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <h4 className="detail-section-title" style={{ margin: 0, border: "none", padding: 0 }}>
                    <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><polygon points="12 2 2 7 12 12 22 7 12 2"></polygon><polyline points="2 17 12 22 22 17"></polyline><polyline points="2 12 12 17 22 12"></polyline></svg>
                    System Architecture
                  </h4>
                  <button onClick={() => setExpandedPanel(expandedPanel === "architecture" ? null : "architecture")} className="btn-arch-toggle" style={{ padding: "6px 12px", fontSize: "0.75rem", background: expandedPanel === "architecture" ? "rgba(6, 182, 212, 0.08)" : "transparent", borderColor: expandedPanel === "architecture" ? "var(--accent-cyan)" : "rgba(6, 182, 212, 0.12)" }}>
                    {expandedPanel === "architecture" ? "Hide Diagram" : "Show Diagram"}
                  </button>
                </div>
                {expandedPanel === "architecture" && <ArchitectureVisualizer name={p.name} />}
              </div>
            )}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
            <div>
              <h4 className="detail-section-title">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path></svg>
                Tech Stack
              </h4>
            ...
            <div>
              <h4 className="detail-section-title">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>
                Project Assets & Links
              </h4>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                {p.hasGithub ? (
                  <a href={p.github} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "10px 18px", fontSize: "0.85rem", textDecoration: "none", flex: 1, justifyContent: "center" }}>
                    <Icons.Github /><span>View Code</span><Icons.ExternalLink />
                  </a>
                ) : (
                  <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, color: "var(--text-muted)", fontSize: "0.85rem", fontStyle: "italic", opacity: 0.75, border: "1px dashed rgba(243, 244, 246, 0.08)", borderRadius: 8, padding: "10px 18px", flex: 1 }}>
                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 2 }}><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <span>Currently Working</span>
                  </div>
                )}
                {p.live && (
                  <a href={p.live} target="_blank" rel="noreferrer" className="btn-primary" style={{ borderRadius: 8, padding: "10px 18px", fontSize: "0.85rem", textDecoration: "none", flex: 1, justifyContent: "center", color: "#030712" }}>
                    ...
                    <span>Live Demo</span><Icons.ExternalLink />
                  </a>
                )}
              </div>
            </div>
            {p.impact && p.impact.length > 0 && (
              <div>
                <h4 className="detail-section-title">
                  <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><line x1="18" y1="20" x2="18" y2="10"></line><line x1="12" y1="20" x2="12" y2="4"></line><line x1="6" y1="20" x2="6" y2="14"></line></svg>
                  Engineering Impact
                </h4>
            ...
            {p.deepDive && (
              <div>
                <h4 className="detail-section-title">
                  <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--accent-cyan)" }}><polyline points="16 18 22 12 16 6"></polyline><polyline points="8 6 2 12 8 18"></polyline></svg>
                  Technical Deep Dive
                </h4>
                <div className="deep-dive-card">
                  <div style={{ position: "absolute", top: 12, right: 16, opacity: 0.08, color: "var(--accent-cyan)", pointerEvents: "none" }}>
                    <svg viewBox="0 0 24 24" width="32" height="32" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 18 22 12 16 6"></polyline><polyline points="8 6 2 12 8 18"></polyline></svg>
                  </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: "40px", borderTop: "1px solid rgba(243, 244, 246, 0.05)", paddingTop: "24px" }}>
          <button className="project-detail-back-btn" onClick={onBack} style={{ margin: 0 }}>← Back to All Projects</button>
        </div>
      </div>
    </div>
  );
...
function SkillGroup({ group, items, index }) {
  ...
  return (
    <FadeIn delay={index * 0.06}>
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="glass-panel" style={{ borderRadius: 14, padding: "24px", height: "100%", transform: hovered ? "translateY(-3px)" : "none", border: `1px solid ${hovered ? "rgba(6, 182, 212, 0.2)" : "rgba(243, 244, 246, 0.05)"}` }}>
        <div style={{ fontSize: "0.8rem", fontFamily: "'Fira Code', monospace", color: "#06B6D4", letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 16, fontWeight: 600 }}>{group}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {items.map(item => (<span key={item} className="tech-badge" style={{ background: "rgba(31, 41, 55, 0.35)", color: "var(--text)" }}>{item}</span>))}
        </div>
      </div>
    </FadeIn>
  );
...
function HighlightCard({ item }) {
  ...
  return (
    <div onMouseMove={handleMouseMove} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="glass-panel glass-panel-glow" style={{ borderRadius: 16, padding: "32px", transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", transform: hovered ? "translateY(-4px)" : "none", border: `1px solid ${hovered ? "rgba(6, 182, 212, 0.25)" : "rgba(243, 244, 246, 0.05)"}`, cursor: "default", height: "100%" }}>
      <div style={{ marginBottom: 18, filter: hovered ? "drop-shadow(0 0 6px rgba(6, 182, 212, 0.3))" : "none", transition: "all 0.2s" }}>{item.icon()}</div>
...
function ExperienceSection() {
  const experience = {
    ...
    location: "Pune, India",
    bullets: [
      "Architected and developed secure, multi-tenant SaaS microservices using Java 17, Spring Boot, Hibernate, and Spring Security (JWT/RBAC) to support international logistics and retail client systems.",
      "Engineered event-driven message pipelines using Apache Kafka and optimized databases (PostgreSQL, MongoDB, Redis), reducing processing delays by 25% and API response latency by 30%.",
      "Built responsive dashboards and user portals using React.js (React 19), Redux, and TypeScript, leveraging pre-tested UI accelerators to improve frontend page load and rendering performance by 40%.",
      "Containerized microservices with Docker, orchestrated deployments via Kubernetes, and provisioned AWS infrastructure (EC2, S3, RDS, IAM, CloudWatch, SNS, SQS, VPC, ECR, ALB) to ensure high availability and scalability across staging and production environments.",
      "Collaborated in an Agile development environment — contributing to sprint planning, daily stand-ups, code reviews, and retrospectives to ensure iterative delivery and consistent team alignment."
    ],
  ...
  return (
    <section id="experience" style={{ position: "relative", zIndex: 2, padding: "100px 6% 60px", background: "rgba(10, 15, 30, 0.3)", borderTop: "1px solid rgba(243, 244, 246, 0.03)", borderBottom: "1px solid rgba(243, 244, 246, 0.03)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <FadeIn>
          <div style={{ fontFamily: "'Fira Code', monospace", fontSize: "0.8rem", color: "#06B6D4", letterSpacing: "2px", textTransform: "uppercase", marginBottom: 12 }}>// work history</div>
          <h2 style={{ fontSize: "clamp(28px, 4vw, 40px)", fontWeight: 700, letterSpacing: "-1px", marginBottom: 48 }}>Professional Experience</h2>
        </FadeIn>

        <FadeIn delay={0.1}>
          <div className="glass-panel" style={{ borderRadius: 20, padding: "clamp(28px, 4vw, 48px)", position: "relative", overflow: "hidden", border: "1px solid rgba(243, 244, 246, 0.06)" }}>
            {/* Top accent line */}
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg, transparent, var(--accent-cyan), transparent)" }} />
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 20, marginBottom: 32 }}>
              <div>
                {/* Role badge */}
                <span style={{ fontSize: "0.7rem", fontFamily: "'Fira Code', monospace", letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--accent-cyan)", background: "rgba(6, 182, 212, 0.08)", border: "1px solid rgba(6, 182, 212, 0.15)", borderRadius: 4, padding: "4px 10px", display: "inline-block", marginBottom: 12 }}>
                  {experience.type}
                </span>
                <h3 style={{ fontSize: "clamp(1.3rem, 2.5vw, 1.8rem)", fontWeight: 800, color: "var(--text)", marginBottom: 6, letterSpacing: "-0.5px", lineHeight: 1.2 }}>
                  {experience.role}
                </h3>
                <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "1.05rem", color: "#06B6D4", fontWeight: 600 }}>
                  <Icons.Briefcase />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(99, 102, 241, 0.08)", border: "1px solid rgba(99, 102, 241, 0.2)", borderRadius: 8, padding: "8px 16px", fontSize: "0.85rem", color: "#a5b4fc", fontFamily: "'Fira Code', monospace" }}>
                ...
                <div style={{ display: "flex", gap: 10, fontSize: "0.82rem", color: "var(--text-muted)" }}>
                  ...
                  <span style={{ color: "var(--accent-cyan)", fontWeight: 600 }}>· {experience.duration}</span>
                </div>
              </div>
            </div>

            {/* Divider */}
            <div style={{ height: 1, background: "rgba(243, 244, 246, 0.05)", marginBottom: 28 }} />

            {/* Bullet Points */}
            <ul style={{ listStyle: "none", padding: 0, margin: "0 0 32px 0", display: "flex", flexDirection: "column", gap: 16 }}>
              {experience.bullets.map((bullet, idx) => (
                <li key={idx} style={{ display: "flex", gap: 14, fontSize: "0.95rem", color: "var(--text-muted)", lineHeight: 1.65 }}>
                  <span style={{ color: "var(--accent-cyan)", fontWeight: 700, flexShrink: 0, marginTop: 2, fontSize: "0.75rem" }}>▹</span>
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
            ...
            {/* Tech Used */}
            <div>
              <div style={{ fontSize: "0.75rem", fontFamily: "'Fira Code', monospace", color: "#06B6D4", letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 12, fontWeight: 600 }}>Technologies Used</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {experience.techUsed.map(t => (<span key={t} className="tech-badge">{t}</span>))}
              </div>
            </div>
          </div>
        </FadeIn>
...
export default function Portfolio() {
  ...
  useEffect(() => { document.body.className = theme === "deepspace" ? "" : `theme-${theme}`; }, [theme]);
  useEffect(() => {
  ...
  const highlightItems = [
    { icon: Icons.Ownership, title: "End-to-End Ownership", desc: "I own features from database modeling and REST API development to polished React frontends. I bridge server-side stability with client-side user experience." },
    { icon: Icons.Ai, title: "AI-Integrated Backends", desc: "Experienced in designing and implementing Retrieval-Augmented Generation (RAG) pipelines, semantic vector search (Qdrant), and LLM text generation via Spring AI." },
    { icon: Icons.Cloud, title: "Cloud & Container Native", desc: "Autonomously deploy containerized architectures (Docker/Kubernetes) and provision AWS cloud resources (EC2, S3, RDS, IAM, CloudWatch, SNS, SQS, VPC, ECR, ALB) to build scalable, highly available systems." },
    { icon: Icons.Lock, title: "Security by Design", desc: "Every feature is secured from day one. I implement Spring Security 6, stateless JWT access/refresh token flows, and Role-Based Access Control (RBAC) on both client and server sides." },
    { icon: Icons.Speed, title: "Performance Aware", desc: "Utilizing Redis caching and distributed locks to prevent race conditions, and Kafka for async message queuing, I write code designed to run efficiently under load." },
    { icon: Icons.Product, title: "Production Mindset", desc: "I focus on maintainable, production-ready code. Writing unit tests (JUnit, Mockito), working in Agile environments, and setting up Docker are part of my daily practices." }
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative", overflowX: "hidden" }}>
      ...
      {/* NAVIGATION */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 6%", height: 72, background: scrolled ? "rgba(3, 7, 18, 0.85)" : "transparent", backdropFilter: scrolled ? "blur(16px)" : "none", borderBottom: scrolled ? "1px solid rgba(243, 244, 246, 0.05)" : "none", transition: "all 0.3s ease" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {navLinks.map(l => (
            <button key={l} onClick={() => scrollTo(l.toLowerCase())} className={`nav-link ${active === l ? "active" : ""}`} style={{ background: "transparent", border: "none", cursor: "pointer", padding: "8px 16px", fontFamily: "'Inter', sans-serif" }}>
              {l}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button onClick={() => { const themes = ["deepspace", "matrix", "cyberpunk"]; setTheme(themes[(themes.indexOf(theme) + 1) % themes.length]); }} className="btn-theme-selector" style={{ fontFamily: "'Fira Code', monospace" }}>
            <span>🎨 Theme:</span>
            <span style={{ color: "var(--accent-cyan)", textTransform: "capitalize", fontWeight: 600 }}>{theme}</span>
          </button>
          <a href={LINKEDIN_URL} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "8px 16px", fontSize: "0.85rem" }}>
            <Icons.LinkedIn /><span>LinkedIn</span>
          </a>
          <a href={GITHUB_URL} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "8px 16px", fontSize: "0.85rem" }}>
      ...
      {/* HERO SECTION */}
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "120px 6% 60px", position: "relative", zIndex: 2 }}>
        <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 64, width: "100%", alignItems: "center" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(6, 182, 212, 0.08)", border: "1px solid rgba(6, 182, 212, 0.2)", borderRadius: 24, padding: "6px 16px", fontSize: "0.85rem", color: "#06B6D4", marginBottom: 28, fontFamily: "'Fira Code', monospace" }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#06B6D4", display: "inline-block", animation: "pulse-slow 2s infinite" }} />
              Open to Opportunities
            </div>
            <h1 style={{ fontSize: "clamp(36px, 5.5vw, 68px)", fontWeight: 800, lineHeight: 1.05, margin: "0 0 16px", letterSpacing: "-2.5px" }}>
              Engineering Scalable<br />
              <span className="text-gradient">Full-Stack Applications</span>
            </h1>
            <p style={{ fontSize: "clamp(18px, 2.5vw, 24px)", fontWeight: 400, color: "var(--text-muted)", marginBottom: 24, fontFamily: "'Outfit', sans-serif" }}>
            ...
            <p style={{ fontSize: "1.05rem", lineHeight: 1.75, color: "var(--text-muted)", maxWidth: 620, marginBottom: 44 }}>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <button onClick={() => scrollTo("projects")} className="btn-primary" style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.95rem" }}>
                <span>Explore Projects</span><span>→</span>
              </button>
              {/* Updated: opens resume in new tab */}
              <a href={RESUME_LINK} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.95rem", textDecoration: "none" }}>
                <Icons.Download />
      ...
      {/* ABOUT SECTION — updated from resume */}
      <section id="about" style={{ position: "relative", zIndex: 2, padding: "100px 6% 60px", background: "rgba(10, 15, 30, 0.3)", borderTop: "1px solid rgba(243, 244, 246, 0.03)", borderBottom: "1px solid rgba(243, 244, 246, 0.03)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <FadeIn>
            ...
            <FadeIn delay={0.1}>
              <div>
                <p style={{ fontSize: "1.05rem", lineHeight: 1.8, color: "var(--text-muted)", marginBottom: 20 }}>
                  I am a Java Full Stack Developer with 3+ years of experience building and shipping end-to-end web applications — from Spring Boot microservices and event-driven backends to responsive React frontends. I specialize in designing secure REST APIs with Spring Security and JWT, managing cloud infrastructure on AWS, and delivering production-ready UIs with React.js, TypeScript, and Tailwind CSS.
                </p>
                <p style={{ fontSize: "1.05rem", lineHeight: 1.8, color: "var(--text-muted)", marginBottom: 20 }}>
                  Currently working at Deqode Solutions Pvt. Ltd. in Pune, I architect multi-tenant SaaS microservices for international logistics and retail clients. My day-to-day spans building event-driven pipelines with Apache Kafka, containerizing services with Docker and Kubernetes, and provisioning AWS infrastructure (EC2, S3, RDS, IAM, CloudWatch, SNS, SQS, VPC, ECR, ALB) across staging and production environments.
                </p>
                <p style={{ fontSize: "1.05rem", lineHeight: 1.8, color: "var(--text-muted)", marginBottom: 20 }}>
                  I am experienced in AI-integrated backends using Spring AI and real-time distributed systems. Recently I have been building Retrieval-Augmented Generation (RAG) platforms, incorporating large language models (LLMs) with vector databases (Qdrant) to design context-aware, semantic search-enabled workspaces.
                </p>
                <p style={{ fontSize: "1.05rem", lineHeight: 1.8, color: "var(--text-muted)", margin: 0 }}>
                  I approach development with a production mindset — designing around clean architecture, code testability (JUnit, Mockito), stateless JWT security, and Agile delivery. I collaborate actively in sprint planning, daily stand-ups, and code reviews.
                </p>
              </div>
            </FadeIn>
            <FadeIn delay={0.2}>
              <div className="glass-panel" style={{ borderRadius: 16, padding: "36px 28px", display: "flex", flexDirection: "column", alignItems: "center", border: "1px solid rgba(243, 244, 246, 0.05)" }}>
                <div className="profile-img-container" style={{ marginBottom: 20 }}>
                  <div className="profile-img-glow" />
                  <img src="https://avatars.githubusercontent.com/u/250507116?v=4" alt="Ritik Hedau" className="profile-img" />
                </div>
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: "0.85rem", color: "var(--text-muted)", textAlign: "left" }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <span>3+ Years · Deqode Solutions</span>
                    </div>
                    <div style={{ display: "flex", gap: 10 }}>
                      <span style={{ color: "#06B6D4" }}>🌐 Portfolio:</span>
                      <a href="https://ritikhedau.vercel.app" target="_blank" rel="noreferrer" style={{ color: "var(--text-muted)", textDecoration: "underline" }}>ritikhedau.vercel.app</a>
                    </div>
                  </div>
                  {/* Quick links */}
                  <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "center" }}>
                    <a href={LINKEDIN_URL} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "8px 14px", fontSize: "0.8rem", textDecoration: "none" }}>
                      <Icons.LinkedIn /><span>LinkedIn</span>
                    </a>
                    <a href={GITHUB_URL} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "8px 14px", fontSize: "0.8rem", textDecoration: "none" }}>
                      <Icons.Github /><span>GitHub</span>
                    </a>
                  </div>
                </div>
      ...
      {/* SKILLS SECTION */}
      <section id="skills" style={{ position: "relative", zIndex: 2, padding: "100px 6% 60px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <FadeIn>
      ...
      {/* PROJECTS SECTION */}
      <section id="projects" style={{ position: "relative", zIndex: 2, padding: "100px 6% 60px", background: "rgba(10, 15, 30, 0.3)", borderTop: "1px solid rgba(243, 244, 246, 0.03)", borderBottom: "1px solid rgba(243, 244, 246, 0.03)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <FadeIn>
          ...
          {selectedProject === null ? (
            <div className="projects-grid">
              {projects.map((p, i) => (
                <ProjectGridCard key={p.name} p={p} index={i} onClick={() => {
            ...
            <ProjectDetailView p={selectedProject} onBack={() => setSelectedProject(null)} />
          )}
        </div>
      </section>

      {/* HIGHLIGHTS SECTION */}
      <section id="highlights" style={{ position: "relative", zIndex: 2, padding: "100px 6% 60px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <FadeIn>
      ...
      {/* CONTACT SECTION */}
      <section id="contact" style={{ position: "relative", zIndex: 2, padding: "100px 6% 120px", background: "rgba(10, 15, 30, 0.3)", borderTop: "1px solid rgba(243, 244, 246, 0.03)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <FadeIn>
          ...
          <FadeIn delay={0.1}>
            <div className="glass-panel" style={{ borderRadius: 24, padding: "56px 40px", textAlign: "center", position: "relative", overflow: "hidden", border: "1px solid rgba(243, 244, 246, 0.05)" }}>
              <div style={{ position: "absolute", top: "30%", left: "50%", transform: "translate(-50%,-50%)", width: "50%", aspectRatio: "1", background: "radial-gradient(circle, rgba(6, 182, 212, 0.07) 0%, transparent 70%)", pointerEvents: "none" }} />
              <h3 style={{ fontSize: "2.2rem", fontWeight: 700, letterSpacing: "-1px", marginBottom: 12, color: "var(--text)" }}>Start a Conversation</h3>
              ...
              <div style={{ marginBottom: 44 }}>
                <a href={MAILTO} style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "16px 32px", border: "1px solid rgba(243, 244, 246, 0.08)", borderRadius: 12, textDecoration: "none", color: "var(--text)", fontSize: "1.05rem", fontFamily: "'Fira Code', monospace", transition: "all 0.3s ease", background: "rgba(17, 24, 39, 0.5)" }}
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--accent-cyan)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(6, 182, 212, 0.15)"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(243, 244, 246, 0.08)"; e.currentTarget.style.boxShadow = "none"; }}>
                  <Icons.Email /><span>{EMAIL}</span><Icons.ExternalLink />
                </a>
              </div>
              <div style={{ display: "flex", justifyContent: "center", gap: 16, flexWrap: "wrap" }}>
                <button className="btn-primary" onClick={copyEmail} style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.9rem" }}>
                  <span>{copied ? "✓ Copied!" : "📋 Copy Email Address"}</span>
                </button>
                <a href={`tel:${PHONE.replace(/\s+/g, '')}`} className="btn-secondary" style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.9rem", textDecoration: "none" }}>
                  <Icons.Phone /><span>{PHONE}</span>
                </a>
                <a href={LINKEDIN_URL} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.9rem", textDecoration: "none" }}>
                  <Icons.LinkedIn /><span>LinkedIn</span>
                </a>
                <a href={GITHUB_URL} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.9rem", textDecoration: "none" }}>
                  <Icons.Github /><span>GitHub</span>
                </a>
                <a href={RESUME_LINK} target="_blank" rel="noreferrer" className="btn-secondary" style={{ borderRadius: 8, padding: "12px 28px", fontSize: "0.9rem", textDecoration: "none" }}>
                  <Icons.Download /><span>View Resume</span>
                </a>
              </div>
      ...
      {/* FOOTER */}
      <footer style={{ textAlign: "center", padding: "32px 5%", borderTop: "1px solid rgba(243, 244, 246, 0.04)", color: "var(--text-muted)", fontSize: "0.85rem", background: "#030712", position: "relative", zIndex: 2 }}>
        <span>Built by <span style={{ color: "#06B6D4", fontWeight: 500 }}>Ritik Hedau</span> — Java Full Stack Developer · Deqode Solutions</span>
      </footer>
    </div>
  );
}